...
def catalog():
    ...
    if 'username' not in login_session:
        return render_template(
            'publicCatalog.html', categories=categories, books=books)
    else:
        return render_template(
            'catalog.html', categories=categories, books=books)
...
def showBook(category_id, book_id):
    ...
    if 'username' not in login_session or \
            books.user_id != login_session['user_id']:
        return render_template(
            'publicBook.html', categories=categories, books=books)
    else:
        return render_template(
            'book.html', categories=categories, books=books)
...
@app.route(
    '/catalog/<int:category_id>/<int:book_id>/edit/', methods=['GET', 'POST'])
def editBook(category_id, book_id):
    ...
    if login_session['user_id'] != editedBook.user_id:
        return "<script>function myFunction() {alert('You are not authorized \
            to edit this book.\
             Please create your own Book Menu in order to edit.\
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            return redirect(url_for(
                'booksMenu', category_id=category_id, book_id=book_id))
        else:
            return render_template(
                'editBook.html',
...
@app.route(
    '/catalog/<int:category_id>/<int:book_id>/delete/',
    methods=['GET', 'POST'])
def deleteBook(category_id, book_id):
    ...
    if deletedBook.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not \
            authorized to delete this book. Please create your own\
            Book Menu in order to delete.');}\
            </script><body onload='myFunction()''>"
    if request.method == 'POST':
